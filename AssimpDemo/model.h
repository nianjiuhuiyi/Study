...
public:
	...
	Model(string const &path, float scale=1.0f,bool gamma = false) : gammaCorrection(gamma) {
...
private:
	...
	void loadModel(string const &path, const float scale) {
		...
		*/
		...
		// 打印更新后的缩放因子
		aiVector3D new_scalingVector = aiVector3D(scene->mRootNode->mTransformation.a1, scene->mRootNode->mTransformation.b2, scene->mRootNode->mTransformation.c3);
		std::cout << "缩放后: " << new_scalingVector.x << "  ##  " << new_scalingVector.y << "  ##  " << new_scalingVector.z << std::endl;
		
	
		std::cout << "Animations-nums: " << scene->mNumAnimations << std::endl;
		std::cout << "materis-nums: " << scene->mNumMaterials << std::endl;
